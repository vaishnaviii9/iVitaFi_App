...
const determineCustomerStandingMessage = (validSummary: AccountSummary) => {
...
const determineButtonVisibility = (validSummary: AccountSummary, paymentSetupData: PaymentSetupData) => {
  const isAccountClosed = validSummary.detail?.creditAccount?.creditApplication?.status === CreditApplicationStatus.AccountClosed;
  const isBankrupt = validSummary.isBankrupt;
  ...
  } else if ((currentAmountDue === 0 && isAutoPay === false && currentBalance > 0) ||
